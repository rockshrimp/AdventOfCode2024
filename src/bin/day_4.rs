...
fn east(x:usize, y:usize, grid: &Vec<String>, word: &str)-> bool{
    //println!("{} {} {} found east match", x, y, &grid[x][y - word.len()..y]);
...
fn detect_word_2(x:usize, y:usize, grid: &Vec<String>) -> bool {
    let word = "MAS";
    let width = grid[0].len();
    let height = grid.len();

    if x > 0 && x + 1 < height && y > 0 && y + 1 < width{ 
        if (north_east(x + 1, y - 1, grid, word) || south_west(x - 1, y + 1, grid, word)) &&
        (south_east(x - 1, y - 1, grid, word) || north_west(x + 1, y + 1, grid, word)) 
        {
            return true;
        }
    }
