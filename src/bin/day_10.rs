...
fn get_trailhead_score_2(pos:Position, grid: &Vec<Vec<i32>>, width: usize, height: usize) -> i32{
    let mut queue: Vec<(i32, i32)> = vec![pos];
    let mut score = 0;
    ...
    while queue.len() > 0{
        ...
        for neighbour in get_neighbours(pos){
            if coordinates_in_bounds(width as i32, height as i32, neighbour){
                if grid[neighbour.0 as usize][neighbour.1 as usize] == val + 1{
                    queue.push(neighbour);
                }
            }
        }
...
fn part_2(){
    ...
    let mut grid: Vec<Vec<i32>> = Vec::new();
    for line in lines{
        grid.push(line.chars().map(|c| if c!= '.' {c.to_digit(10).unwrap() as i32} else {11}).collect());
    }
    ...
    for (x, line) in grid.iter().enumerate(){
        ...
        for (y, val) in line.iter().enumerate(){
            if *val == 0{
                //println!("0 found at {},{}", x, y);
                scores_sum += get_trailhead_score_2((x as i32, y as i32), &grid, width, height);
            }
        } 
    }
